...
- **语言偏好**：`summary` 的默认语言应跟随项目或仓库的 `summary_language` 偏好；未单独指定时，默认使用中文。
- **summary 要求**：无论最终采用单行还是多行，`summary` 都应直接表达功能点变化、用户可感知结果或明确的业务影响；允许保留必要的代码层动作词，但必须落到具体功能点，避免“修复问题”“优化代码”这类空泛描述。
- **多行偏好**：提交应优先跟随项目或仓库的 `body_style` 偏好；当改动仍属同一目标，但需要补充关键影响面、行为边界、兼容性或主要变化维度时，可使用简短 body。
- **body 建议**：多行 body 建议控制在 3 行内，优先说明关键变化维度，不要写成文件清单或流水账。
- **提交流程限制**：commit-only 任务默认不改代码；若确实需要改，至少先询问用户。
- **提交边界**：一次 commit 只提交同一目标的改动，禁止把无关修改打包在一起；如果改动里出现多条相对独立的故事线，优先考虑拆分建议，必要时再保留在同一 commit 并通过简短 body 补充说明。
...
也就是 `summary_language` 和 `body_style` 都属于偏好项：前者决定默认语言，后者决定单行还是多行更合适，而不是把单行当成固定默认。当一个提交仍然只表达一件事，但需要补充关键影响面、行为边界、兼容性或主要变化维度时，可以追加简短 body。哪怕只有 1 个关键维度，只要它的变动幅度足够大、缺少补充会让人误解，也可以使用多行。若项目本身长期使用英文提交，可在项目规则里把 `summary_language` 改成 `en`。
